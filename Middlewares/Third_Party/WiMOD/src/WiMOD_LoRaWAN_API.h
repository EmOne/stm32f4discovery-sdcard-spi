...
typedef struct
{
  ...
  uint8_t linkADR;
  uint8_t opMode;
  uint8_t pwrTransmission;
  uint8_t dateRate;
  uint8_t maxPlayloadSize;
  uint8_t playloadSize;
  uint8_t err_idx;
  char* err_desc;
  uint32_t period;
} LoRa_App;
...
#define	LORAWAN_STATUS_CHANNEL_BLOCKED_BY_DC	0x0A	//Channel blocked by Duty Cycle
...
int
WiMOD_LoRaWAN_SendURadioData(UINT8 port, UINT8* data, size_t length);

// send confirmed radio data
int
WiMOD_LoRaWAN_SendCRadioData(UINT8 port, UINT8* data, size_t length);
...
__weak int WiMOD_Error_Handler(void* obj);
__weak int loraDataRx(uint8_t fport, uint8_t* data, size_t len);

extern LoRa_App loraAppStatus;
