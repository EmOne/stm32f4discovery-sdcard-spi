...
//
//------------------------------------------------------------------------------
int function_wrapper(int (*f)(void))
{
  uint32_t timeout = 1000000;
  int ret = 0;
  if(ret = f() == 1)
  {
    while (!(TxMessage.SapID == RxMessage.SapID))
    {
      if(--timeout) continue;
      return 0;
    }
    while (!(RxMessage.MsgID == TxMessage.MsgID + 1))
    {
      if(--timeout) continue;
      return 0;
    }
  ...
  int ret = 0 ;
	...
	switch (msg_id) {
		...
		case DEVMGMT_MSG_SET_OPMODE_REQ:
			...
			break;
		default:
                ret = DEVMGMT_STATUS_CMD_NOT_SUPPORTED;
			return ret;
	}
	...
	if(TxMessage.Length > 0)
        ...
        RxMessage.State = TxMessage.State = 1;
...
int WiMOD_LoRaWAN_Msg_Req(uint8_t msg_id, uint8_t* val, uint16_t len)
{
  int ret = 0;
	...
	if(TxMessage.Length > 0)
        ...
        RxMessage.State = TxMessage.State = 1; //Response
...
// set mac cmd
int
WiMOD_LoRaWAN_SetMAC_CMD(uint8_t * payload, int len){
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_SEND_MAC_CMD_REQ, payload, len);
}
...
WiMOD_LoRaWAN_Process_RxMessage(TWiMOD_HCI_Message*  rxMessage)
{
    ...
    if(rxMessage->Payload[0] != LORAWAN_STATUS_OK)
          WiMOD_Error_Handler(rxMessage);

    TxMessage.State = RxMessage.State = 0;
    return &RxMessage;
}
...
WiMOD_LoRaWAN_DevMgmt_Get_OPMODE_Rsp(TWiMOD_HCI_Message* rxMessage)
{
	...
	{
          loraAppStatus.opMode = rxMessage->Payload[1];
...
WiMOD_LoRaWAN_DevMgmt_DeviceInfo_Rsp(TWiMOD_HCI_Message*  rxMessage)
{
    ...
    if (rxMessage->Payload[0] == DEVMGMT_STATUS_OK)
    {
        ...
        memcpy((uint8_t *) &help, &rxMessage->Payload[6], 4);
        loraAppStatus.devAddr.u32 = loraAppStatus.devID.u32 = help;
        printf("Device ID: 0x");
		printf((const char* ) num2hex((uint32_t)help, DOUBLEWORD_F));
		printf("(");
		num2str(help, str);
...
WiMOD_LoRaWAN_Process_Reactivate_Rsp(TWiMOD_HCI_Message*  rxMessage)
{
    ...
    if (rxMessage->Payload[0] == LORAWAN_STATUS_OK)
    {
        ...
        loraAppStatus.devAddr.u32 = help;
...
WiMOD_LoRaWAN_Process_U_DataRxIndication(TWiMOD_HCI_Message* rxMessage)
{
    ...
    if (payloadSize >= 1)
    {
        ...
        loraDataRx(rxMessage->Payload[1], &rxMessage->Payload[2], payloadSize);
    }
...
WiMOD_LoRaWAN_Process_Get_Network_Status_RSP(TWiMOD_HCI_Message* rxMessage)
{
	...
	if (rxMessage->Payload[0] == LORAWAN_STATUS_OK) {
            loraAppStatus.nwkStatus = rxMessage->Payload[1];
		...
		printf((const char* ) num2hex((uint32_t)help, DOUBLEWORD_F));
		printf("\n\r");
            loraAppStatus.dateRate = rxMessage->Payload[6];
		num2str(rxMessage->Payload[6],str);
		printf("\tDR: ");
		printf(str);
		printf("\n\r");
            loraAppStatus.pwrTransmission = rxMessage->Payload[7];
		num2str(rxMessage->Payload[7],str);
		printf("\tPower: ");
		printf(str);
		printf("dBm\n\r");
            loraAppStatus.maxPlayloadSize = rxMessage->Payload[8];
...
WiMOD_LoRaWAN_Process_Get_Device_EUI_RSP(TWiMOD_HCI_Message* rxMessage)
{
	...
	if (rxMessage->Payload[0] == LORAWAN_STATUS_OK)
	{
          memcpy(&loraAppStatus.devEUI[0], &rxMessage->Payload[1], 8);
		printf("Device EUI: \n\r");
...
WiMOD_LoRaWAN_ShowResponse(const char* string, const TIDString* statusTable, UINT8 statusID)
{
    while(statusTable->String)
    {
        ...
        {
          loraAppStatus.err_idx = statusID;
          loraAppStatus.err_desc = (char *) statusTable->String;
...
//
//------------------------------------------------------------------------------
int GetNwkStatus(void) {
	printf("GetNwkStatus\r\n");

	// send get network status message
	return WiMOD_LoRaWAN_GetNkwStatus();
}
...
//
//------------------------------------------------------------------------------
int SetCustomCFG(void) {
	int8_t payload = 0; //dBd
	printf("SetCustomCFG\n\r");

	// send set custom config message
	return WiMOD_LoRaWAN_SetCustomConfig(payload);
}
...
//
//------------------------------------------------------------------------------
int GetCustomCFG(void) {
	printf("GetCustomCFG\n\r");

	// send get custom config message
	return WiMOD_LoRaWAN_GetCustomConfig();
}
...
//
//------------------------------------------------------------------------------
int ActivateABP(void) {
	uint8_t payload[37] = {};

	printf("Activate ABP\n\r");
        memcpy(&payload[0], loraAppStatus.devAddr.u8, 4);
        memcpy(&payload[4], loraAppStatus.nwkSKey, 16);
        memcpy(&payload[20], loraAppStatus.appSKey, 16);
	// send unconfirmed radio message
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_ACTIVATE_DEVICE_REQ, payload, 37);
}
...
int Reactivate(void) {
	...
	// send unconfirmed radio message
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_REACTIVATE_DEVICE_REQ, NULL, 0);
}
...
int Deactivate(void) {
	...
	// send unconfirmed radio message
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_DEACTIVATE_DEVICE_REQ, NULL, 0);
}
...
//
//------------------------------------------------------------------------------
int GetRadioStack(void) {
	printf("GetRadioStack\n\r");
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_GET_RSTACK_CONFIG_REQ, NULL, 0);
}
//------------------------------------------------------------------------------
//
//  SetRadioStack
...
//	    Bit 7: 0 = Rx MAC Command Forwarding disabled 	    1 = Rx MAC Command Forwarding enabled
	...
	// send unconfirmed radio message
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_SET_RSTACK_CONFIG_REQ, data, 7);
}
...
int GetDevEUI(void) {
	printf("GetDevEUI\n\r");
	return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_GET_DEVICE_EUI_REQ, NULL, 0);
}
...
//	WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_SET_DEVICE_EUI_REQ, &UsartTextString[2], 8);
        return WiMOD_LoRaWAN_Msg_Req(LORAWAN_MSG_SET_DEVICE_EUI_REQ, &loraAppStatus.devEUI, 8);
}
//------------------------------------------------------------------------------
//
//  SetOPMODE
...
int SetOPMODE(int mode) {
	printf("set OPMODE\n\r");
        if(mode != 3) mode = 0;
	return WiMOD_LoRaWAN_SetOPMODE(mode);
}
...
//
//------------------------------------------------------------------------------
int ClearOPMODE(void) {
	printf("clear OPMODE\n\r");

	return WiMOD_LoRaWAN_SetOPMODE(0);
}
...
int GetOPMODE(void) {
	printf("get OPMODE\n\r");
	return function_wrapper(WiMOD_LoRaWAN_GetOPMODE);
}
...
//
//------------------------------------------------------------------------------
int ClearRTCAlarm(void) {
	printf("clear RTC alarm\n\r");
	return function_wrapper(WiMOD_LoRaWAN_ClearRTCAlarm);
}
...
int GetRTC(void) {
	printf("get RTC\n\r");
	return function_wrapper(WiMOD_LoRaWAN_GetRTC);
}
...
int SetRTC(void) {
	...
	return WiMOD_LoRaWAN_SetRTC(&time);
}
...
int GetAlarm(void) {
	...
	return function_wrapper(WiMOD_LoRaWAN_GetRTCAlarm);
}
...
int SetAlarm(void) {
	...
	payload[2] = 30;	//Minute
	payload[3] = 30;	//Second
	return WiMOD_LoRaWAN_SetRTCAlarm(payload);
}
...
int Ping(void) {
  printf("ping request\n\r");
  return function_wrapper(&WiMOD_LoRaWAN_SendPing);
}
...
int GetFirmwareVersion(void) {
	printf("get firmware version\n\r");
        return function_wrapper(WiMOD_LoRaWAN_GetFirmwareVersion);
}
...
int GetDeviceInfo(void) {
	printf("get device information\n\r");
        return function_wrapper(WiMOD_LoRaWAN_GetDeviceInfo);
}
...
int GetDeviceStatus(void) {
  ...
  function_wrapper(WiMOD_LoRaWAN_GetDeviceStatus);
  return RxMessage.Payload[0];
}
...
{
	printf("reset device\n\r");
    return function_wrapper(WiMOD_LoRaWAN_Reset);
}
...
int Join(void) {
	...
	return function_wrapper(WiMOD_LoRaWAN_JoinNetworkRequest);
}
//------------------------------------------------------------------------------
//
//  SendUData
...
int FactoryReset(void) {
	...
	return function_wrapper(WiMOD_LoRaWAN_FactoryReset);
}
...
int GetLinkADR(void) {
	printf("GetLinkADR\n\r");
	return function_wrapper(WiMOD_LoRaWAN_GetLinkADR);
}
...
//	2: KPN/Actility proposal
	return WiMOD_LoRaWAN_SetLinkADR(&payload);
}
//------------------------------------------------------------------------------
// end of file
//------------------------------------------------------------------------------
